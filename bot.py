...
async def on_message(message):
    ...
    if isinstance(channel, discord.TextChannel) and channel.is_news():
        ...
        if is_message_banned(message):
            print(f"Üzenet kihagyva: {message.id}")
        else:
            now = datetime.utcnow()
            cid = channel.id
...
async def dbhelp2(ctx):
    ...
    if not help_lines:
        await ctx.send("❌ Nem található a help2.txt!")
        return
...
async def dbidlist(ctx):
    ...
    server_names = [server_lines[i] for i in range(0, len(server_lines)-1, 2)]

    user_lines = load_txt("userid.txt")
    user_names = [user_lines[i] for i in range(0, len(user_lines)-1, 2)]
...
# ---------- MINI WEBSERVER (FIXED) ----------
app = Flask(__name__)

@app.route("/")
def home():
    return "OK", 200  # EZ KELL AZ UPTIMEROBOTNAK

def run_webserver():
    port = int(os.environ.get("PORT", 10000))  # FONTOS
